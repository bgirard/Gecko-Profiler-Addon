...
Profiler.prototype = {
  ...
  getProfile: function profiler_getProfile(cb) {
    // If supported, grab the profiles asynchronously to
    // let us get subprocess samples.
    if (this._profiler.getProfileDataAsync) {
      this._profiler.getProfileDataAsync().then(cb.bind(this), console.error);
    } else {
...
function sps_startup(force, manifest) {
    ...
    prefs.default_init_pref("profiler.", "java", false);
    ...
    if (settingsVersion < 8) {
      prefs.set_pref_string("profiler.", "symbolicationUrl", DEFAULT_SYMBOLICATION_URL);
      prefs.set_pref_int("profiler.", "version", 8);
    }
    ...
    var want_feature_java = prefs.get_pref_bool("profiler.", "java", false);
...
function inflateSchemaProfileThreads(profile) {
  ...
  function inflateOptimizations(optimizations, stringTable) {
    ...
    for (let i = 0; i < types.length; i++) {
      ...
      let typeset = type.typeset;
      let inflatedTypeset;

      if (typeset) {
        inflatedTypeset = new Array(typeset.length);
        for (let j = 0; j < typeset.length; j++) {
          let ti = typeset[j];
          inflatedTypeset[j] = {
    ...
    let attempts = optimizations.attempts;
    let attemptsData = attempts.data;
    let inflatedAttempts = new Array(attemptsData.length);

    const ATTEMPT_STRATEGY_SLOT = attempts.schema.strategy;
    const ATTEMPT_OUTCOME_SLOT = attempts.schema.outcome;

    for (let i = 0; i < attemptsData.length; i++) {
      let attempt = attemptsData[i];
      inflatedAttempts[i] = {
  ...
  function inflateSamples(samples, stackTable, frameTable, stringTable, inflatedFramesCache) {
    ...
    const SAMPLE_RESPONSIVENESS_SLOT = samples.schema.responsiveness;
    const SAMPLE_RSS_SLOT = samples.schema.rss;
    const SAMPLE_USS_SLOT = samples.schema.uss;
    const SAMPLE_FRAMENUMBER_SLOT = samples.schema.frameNumber;
    const SAMPLE_POWER_SLOT = samples.schema.power;

    const STACK_PREFIX_SLOT = stackTable.schema.prefix;
    const STACK_FRAME_SLOT = stackTable.schema.frame;

    const FRAME_LOCATION_SLOT = frameTable.schema.location;
    const FRAME_IMPLEMENTATION_SLOT = frameTable.schema.implementation;
    ...
    let inflatedSamples = new Array(samplesData.length);
    ...
    for (let i = 0; i < samplesData.length; i++) {
      ...
      while (stackIndex !== null) {
        ...
        let f = inflatedFramesCache[frameIndex];
        if (!f) {
  ...
  function inflateMarkers(markers, stackTable, frameTable, stringTable, inflatedFramesCache) {
    let MARKER_NAME_SLOT = markers.schema.name;
    let MARKER_TIME_SLOT = markers.schema.time;
    let MARKER_DATA_SLOT = markers.schema.data;

    let markersData = markers.data;
    let inflatedMarkers = new Array(markersData.length);

    for (let i = 0; i < markersData.length; i++) {
      let marker = markersData[i];

      let payload = marker[MARKER_DATA_SLOT];
  ...
  for (let i = 0; i < profile.threads.length; i++) {
    ...
    if (typeof thread == "string") {
      let subprocessProfile = JSON.parse(thread);
    ...
    let numFrames = frameTable.data.length;
    let inflatedFramesCache = new Array(numFrames);
...
let FirefoxAppWrapper = {
  ...
  get_profiler_status: function ff_profiler_status() {
      ...
      button.icon = data.url("toolbar_" + iconType + ".png");
      var profilerStatus = {
