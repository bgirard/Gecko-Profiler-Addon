...
// Don't block the event loop longer than 8ms.
const kChunkLengthMS = 8;

// Compute a map of libraries to resolve
function findSymbolsToResolve(lines, finishCallback) {
    runAsContinuation(function(resumeContinuation) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (Date.now() - lastTime >= kChunkLengthMS) {
                yield timers.setTimeout(resumeContinuation, 0);
                lastTime = Date.now();
            }
        }
        finishCallback(symbolsToResolve);
    });
}
...
function symbolicate(profile, platform, progressCallback, finishCallback) {
    ...
    runAsContinuation(function (resumeContinuation) {
        ...
        var symbolsToResolve = yield findSymbolsToResolve(lines, resumeContinuation);
        var resolvedSymbols = yield resolveSymbols(symbolsToResolve, platform, resumeContinuation);
        var fixedLines = yield substituteSymbols(lines, resolvedSymbols, resumeContinuation);
