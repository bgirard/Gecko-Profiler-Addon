...
//var DEFAULT_UI_URL = "file:///Users/markus/code/cleopatra/index.html";
...
var panel = null;
var c = 0;
var widgetDisplayText = "";

function setWidgetDisplayText(text) {
    widgetDisplayText = text;
    spsWidget.port.emit("displayText", text);
}
...
function sps_update_status() {
    ...
    var addonLabel = profiler.IsActive() ? "Running" : "Stopped";
    setWidgetDisplayText("Profiler: " + addonLabel);
    panel.port.emit("changeStatus", addonLabel.toLowerCase());
    dump("updated status\n")
}
...
function get_profile(progress_callback, finish_callback) {
    ...
    timers.setTimeout(function() {
        ...
        symbolicateModule.symbolicate(profile, progress_callback, finish_callback);
    }, 0);
    return;
}
...
function init() {
    ...
    panel.port.on("startProfiler", function() {
        if (!profiler.IsActive()) {
            sps_startup(true);
        }
        sps_update_status();
    });
    panel.port.on("stopProfiler", function() {
        if (profiler.IsActive()) {
            profiler.StopProfiler();
        }
        sps_update_status();
    });
    panel.port.on("getprofile", function() {
        open_cleopatra();
        panel.hide();
    });

    var hh = Cc["@mozilla.org/network/protocol;1?name=http"].getService(Ci.nsIHttpProtocolHandler);
    if (hh["platform"] == "Macintosh")
        panel.port.emit("platform", "mac");

    spsWidget = require("widget").Widget({
      id: "profiler-display",
      label: "Built-in Profiler",
      contentURL: data.url("sps_toolbarwidget.html"),
      contentScriptFile: data.url("sps_toolbarwidget.js"),
      width: 120,
      panel: panel,
    });

    spsWidget.port.on("ready", function () {
        var hh = Cc["@mozilla.org/network/protocol;1?name=http"].getService(Ci.nsIHttpProtocolHandler);
        if (hh["platform"] == "Macintosh")
            spsWidget.port.emit("platform", "mac");
        spsWidget.port.emit("displayText", widgetDisplayText);
    });
    ...
    try {
        ...
        spsWidget.width = 200;
        setWidgetDisplayText("Profiler module not found.");
        return;
    }
    try {
