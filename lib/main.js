...
var cmdRunnerModule = Cu.import(data.url("CmdRunner.jsm"));
...
function get_feature_pref() {
    ...
    for (var i = 0; i < profilerFeatures.length; i++) {
        ...
        featurePrefs[feature] = get_pref_bool("profiler.", feature);
    }
    return featurePrefs;
}
...
function sps_startup(force) {
    ...
    if (get_pref_bool("profiler.", "enabled") || force == true) {
        ...
        var want_feature_stackwalk = get_pref_bool("profiler.", "stackwalk", true);
        var want_feature_jank = get_pref_bool("profiler.", "jank", false);
        var want_feature_adb = get_pref_bool("profiler.", "adb", false);
...
function adb_get_process_info(finish_callback) {
    cmdRunnerModule.runCommand("adb shell ps", function(r) {
        var lines = r.split("\n");
        var fennecLine = null;
...
function adb_pull(progress_callback, finish_callback) {
    ...
    cmdRunnerModule.runCommand("adb version", function(r) {
        if (r.indexOf("Android Debug Bridge") != 0) {
            progress_callback({progress: 0, action: "Could not find 'adb', make sure it is in your PATH"});
            return;
        }
        progress_callback({progress: 0, action: "Check Env: Android device"});
        cmdRunnerModule.runCommand("adb shell pwd", function(r) {
            ...
            progress_callback({progress: 0, action: "Check Env: Fennec process"});
            adb_get_process_info( function (processInfo) {
                if (processInfo.error != null) {
                    progress_callback({progress: 0, action: processInfo.error});
                    return
                }
                progress_callback({progress: 0, action: "Pulling profile: Send signal to pid: " + processInfo.pid});
                cmdRunnerModule.runCommand("adb shell run-as " + processInfo.pkgName + " kill -12 " + processInfo.pid, function(r) {
                    adb_get_process_info( function (processInfo2) {
                        if (processInfo2.error != null) {
                            ...
                            // Restart the process.
                            progress_callback({progress: 0, action: "Restarting Fennec with profiling enabled."});
                            cmdRunnerModule.runCommand("adb shell am start -n " + processInfo.pkgName + "/.App --es env0 MOZ_PROFILER_STARTUP=true", function(r) {
                                dump("adb shell am start -n " + processInfo.pkgName + "/.App --es env0 MOZ_PROFILER_STARTUP=true\n");
                                progress_callback({progress: 0, action: "Restarted Fennec with profiling enabled."});

                            });
                            return;
                        }
                        progress_callback({progress: 0, action: "Waiting for save"});
                        cmdRunnerModule.runCommand("sleep 3", function (r) {
                            progress_callback({progress: 0, action: "Pulling profile: adb pull"});
                            cmdRunnerModule.runCommand("adb pull /sdcard/profile_0_" + processInfo.pid + ".txt /tmp/fennec_profile.txt", function (r) {
                                progress_callback({progress: 0, action: "Pulling profile: reading"});
                                cmdRunnerModule.runCommand("adb shell rm /sdcard/profile_0_" + processInfo.pid + ".txt", function (r) {});
                                cmdRunnerModule.runCommand("/bin/cat /tmp/fennec_profile.txt", function (r) {
                                    progress_callback({progress: 0, action: "Pulling profile: Got profile: " + r.length});
                                    dump("Profile: '" + r + "'\n");
                                });
                            });
    ...
    tabs.open({
        ...
        onReady: function onReady(tab) {
            ...
            attach.port.on("set_feature", function(obj) {
                set_pref_bool("profiler.", obj.feature, obj.value);
                sps_update_status();
            });
...
function default_init_pref(branch, node, initValue) {
    if (get_pref_bool(branch, node, "null") === "null") {
        set_pref_bool(branch, node, initValue);
    }
}
