...
function GCStats() {
  ...
  function receiveGCStat(subject, data) {
    ...
    parsedEvent.timestamp = parsedEvent.timestamp / 1000 - gProfileStartEpoc;
    for (var i = 0; i < parsedEvent.slices.length; i++) {
      if (!parsedEvent.slices[i].start_timestamp)
        continue;
      parsedEvent.slices[i].start_timestamp = parsedEvent.slices[i].start_timestamp / 1000 - gProfileStartEpoc;
      parsedEvent.slices[i].end_timestamp = parsedEvent.slices[i].end_timestamp / 1000 - gProfileStartEpoc;
    }
...
function sps_startup(force, manifest) {
    ...
    if (prefs.get_pref_bool("profiler.", "enabled") || force == true) {
        ...
        gProfileStartEpoc = new Date().getTime();
...
function adb_get_process_info(finish_callback) {
    cmdRunnerModule.runCommand("/bin/bash -l -c 'adb shell ps'", function(r) {
        ...
        if (pkgName.indexOf("/system/b2g/b2g") != -1) {
            ...
            startCmd = "adb shell stop b2g && adb shell MOZ_PROFILER_STARTUP=true /system/bin/b2g.sh > /dev/null &";
