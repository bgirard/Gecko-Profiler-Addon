...
function sps_startup(force) {
    ...
    default_init_pref_string("profiler.", "format", DEFAULT_PROFILE_FORMAT);
    default_init_pref_string("profiler.", "url", DEFAULT_UI_URL);
                        ...
                        var intervalId = timers.setInterval(function() {
                            ...
                            cmdRunnerModule.runCommand("/bin/bash -l -c 'adb pull /sdcard/profile_0_" + processInfo.pid + ".txt /tmp/fennec_profile.txt'", function (r) {
                                ...
                                cmdRunnerModule.runCommand("/bin/cat /tmp/fennec_profile.txt", function (r) {
                                    symbolicateModule.symbolicate(r, "Linux", null, progress_callback, finish_callback);
                                    //dump("Profile: '" + r + "'\n");
                                    //finish_callback(r);
                                });
                            });
...
function set_pref_string(branch, node, value) {
    var prefs = Cc["@mozilla.org/preferences-service;1"]
                    .getService(Ci.nsIPrefService).getBranch(branch);
...
function default_init_pref_string(branch, node, initValue) {
    if (get_pref_string(branch, node, "null") === "null") {
        set_pref_string(branch, node, initValue);
    }
}
