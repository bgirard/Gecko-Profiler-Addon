...
function adb_pull_libs(progress_callback, finish_callback) {
    cmdRunnerModule.runCommand("/bin/bash -l -c 'adb shell ls /system/lib | sed \"s;^;/system/lib/;\" && adb shell ls /system/vendor/lib/egl | sed \"s;^;/system/vendor/lib/egl/;\"'", function(r) {
        var lines = r.split("\n");
        var libsToPull = [];
...
let ThunderbirdAppWrapper = {
  init: function tb_init() {
    ...
    let dumpButton = doc.createElementNS(
      'http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul',
      'statusbarpanel');

    dumpButton.setAttribute('label', 'Dump Profile');
    dumpButton.setAttribute('id', 'dumpProfile');
    dumpButton.addEventListener('click', function(aEvent) {
      this.open_cleopatra();
    }.bind(this));
    statusBar.appendChild(dumpButton);
  ...
  sps_update_status: function tb_sps_update_status() {
    let isActive = profiler.IsActive();
    console.log("Profiler active?: " + isActive);
  },
