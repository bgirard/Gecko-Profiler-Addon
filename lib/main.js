...
function EnableShutdownWatcher(profiler) {
  ...
  var shutdownObserver = {
    observe: function (subject, topic, data) {
      ...
      stop_profiler(null, false, function stop_cb() {
        sps_startup(true, {
          interval: 10,
          entries: 80000,
        });
...
function IPCStats(profiler) {
  var want_feature_ipc = prefs.get_pref_bool("profiler.", "ipc", true);
  ...
  this.profiler.sendObservableEvent("ipdl-tracing-start");

  var self = this;
  var IPCStatObserver = {
    observe: function (subject, topic, data) {
      console.log("got IPC info: " + JSON.stringify(data));
    }
  };

  profiler.registerEventNotifications(["ipdl-tracing-capture-completed"], IPCStatObserver);
}
IPCStats.prototype = {
  shutdown: function GCStats_shutdown() {
  },
  ...
  getData: function GCStats_getData() {
    this.profiler.sendObservableEvent("ipdl-tracing-capture");
    return {
    };
  },
};
...
function Profiler(options, cb) {
  ...
  this.ipcStats = new IPCStats(this);
...
Profiler.prototype = {
  start: function profiler_start(entries, interval, features, threads, cb) {
    ...
    if (has_feature('threadfilter') && threads.length) {
  ...
  sendObservableEvent: function profiler_sendObservableEvent(eventName, msg) {
    dump("SENDING: " + eventName + "\n");
    Services.obs.notifyObservers(null, eventName, msg);
  },
...
function stop_profiler(isPrivate, forceCache, save_callback) {
  if (isPrivate != true && forceCache != false && prefs.get_pref_bool("profiler.", "copyProfileOnStop", true) ||
    ...
    profiler.getProfile(function(profile) {
      ...
      profile.meta.ipcStats = profiler.ipcStats.getData();
...
function sps_startup(force, manifest) {
    ...
    if (settingsVersion < 6) {
      prefs.set_pref_string("profiler.", "threadfilter", "");
    ...
    appWrapper.setPrivateBrowsingRestartCallback(function pb_restart_callback() {
      sps_startup(true, manifest); 
    });
...
let ThunderbirdAppWrapper = {
  ...
  setPrivateBrowsingRestartCallback: function tb_setPrivateBrowsingRestartCallback(cb) {
  },
...
let FirefoxAppWrapper = {
  ...
  initProfiler: function ff_initProfiler(ProfilerWrapper, options, cb) {
    ...
    try {
      ...
      new ProfilerWrapper(options, function() {
        profiler = this;
        this.registerEventNotifications(["profiler-started", "profiler-stopped", "profiler-locked", "profiler-unlocked"], that);
  ...
  setPrivateBrowsingRestartCallback: function ff_setPrivateBrowsingRestartCallback(cb) {
    this._profilerUnlockCB = cb;
  },
  ...
  observe: function ff_observe(subject, topic, data) {
    if (topic == "profiler-started")
      ...
      profilerIsActive = false;
    else if (topic == "profiler-unlocked") {
      if (this._profilerUnlockCB) {
        this._profilerUnlockCB();
      } else {
        dump("Profiler error: No unlock callback\n");
      }
    }
    appWrapper.sps_update_status();
  },
