...
let ThunderbirdAppWrapper = {
  ...
  initProfiler: function tb_initProfiler(type, cb) {
    var that = this;
    try {
      new type(function() {
        profiler = this;
...
let FirefoxAppWrapper = {
  initProfiler: function ff_initProfiler(type, cb) {
    var that = this;
    try {
      new type(function() {
        profiler = this;
        ...
        if (profilingStartup) {
          ...
          if (obService) {
            console.error("Wait for session restore to dump profile.");
            function receiveSessionRestore() {
              // Can't get this to fire :(
              console.error("Got session restore event");
              analyze();
            }
            service.addObserver(receiveSessionRestore, "browser-delayed-startup-finished", false);
            service.addObserver(receiveSessionRestore, "sessionstore-windows-restored", false);
            //obService.add("browser-delayed-startup-finished", receiveSessionRestore, this);
          }
        }
      ...
      console.error("Profiler module not found: " + e.stack);
    }
  },
