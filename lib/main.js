    ...
    cmdRunnerModule.runCommand("adb version", function(r) {
        ...
        cmdRunnerModule.runCommand("adb shell pwd", function(r) {
            ...
            cmdRunnerModule.runCommand("adb shell ps", function(r) {
                ...
                if (fennecLine == null) {
                    progress_callback({progress: 0, action: "Fennec is not running."});
                    return;
                }
                ...
                if (pid == null || isNaN(pid)) {
                    progress_callback({progress: 0, action: "Check Env: Could not find PID in: " + fennecLine});
                    return;
                }
                if (pkgName.indexOf("org.mozilla") != 0) {
                    progress_callback({progress: 0, action: "Check Env: Could not find package name: " + pkgName});
                    return;
                }
                progress_callback({progress: 0, action: "Pulling profile: Send signal to pid: " + pid});
                cmdRunnerModule.runCommand("adb shell run-as " + pkgName + " kill -12 " + pid, function(r) {
                    dump(r + "\n");
