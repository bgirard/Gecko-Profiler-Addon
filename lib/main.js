                ...
                cmdRunnerModule.runCommand("adb shell run-as " + processInfo.pkgName + " kill -12 " + processInfo.pid, function(r) {
                    adb_get_process_info( function (processInfo2) {
                        if (processInfo2.error != null) {
                            ...
                            cmdRunnerModule.runCommand("Start with profiling: adb shell am start -n " + processInfo.pkgName + "/.App --es env0 MOZ_PROFILER_STARTUP=true", function(r) {
                        ...
                        var intervalId = timers.setInterval(function() {
                            waitCount += 50;
                            progress_callback({progress: (0.2 + waitCount/3000 * 0.7), action: "Waiting for save"});
                            if (waitCount < 3000) {
                                return;
                            }
                            ...
                            cmdRunnerModule.runCommand("adb pull /sdcard/profile_0_" + processInfo.pid + ".txt /tmp/fennec_profile.txt", function (r) {
                                progress_callback({progress: 0.9, action: "Pulling profile: reading"});
