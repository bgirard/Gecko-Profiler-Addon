...
Profiler.prototype = {
  ...
  start: function profiler_start(entries, interval, features, threads, success_cb, fail_cb) {
    //console.log(new Error("myError").stack);
    var result = this._profiler_start(entries, interval, features, threads);
    if (result === true) {
      if (success_cb) success_cb.call(this, result);
    } else {
      if (fail_cb) fail_cb.call(this, result);
    }
  },
    ...
    if (prefs.get_pref_bool("profiler.", "enabled") || force == true) {
        ...
        profiler.start(entries, interval, selectedFeatures, threadFilter.split(','), function() {
            ...
            if (gcli_binding && appWrapper.get_profiler_status) {
                gcli_binding.bindGCLI(profiler, appWrapper.get_profiler_status(), stop_profiler, set_feature, analyze);
            }
        }, function(err) {
          if (err.name == 'NS_ERROR_NOT_AVAILABLE' ) {
            panel.port.emit("private_browsing_failure", true);
          }
        });
    } else {
       appWrapper.registerConsoleProfilerObserver();
       appWrapper.sps_update_status();
       if (gcli_binding && appWrapper.get_profiler_status) {
           gcli_binding.bindGCLI(profiler, appWrapper.get_profiler_status(), function(){sps_startup(true);}, set_feature, analyze);
       }
    }
}
