...
function open_conn() {
  ...
  let client = new DebuggerClient(debuggerSocketConnect(host, port));
...
function init() {
  ...
  if (true || prefs.get_pref_bool("profiler.", "force_dbg_server", false)) {
    try {
      ...
      DebuggerServer.openListener(6010, true);
      dump("Start on port 6010\n");
      return;
      open_conn();
    } catch(e) {
