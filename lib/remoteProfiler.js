...
function RemoteProfiler(options, cb) {
  ...
  this._sharedLibraryInformation = [];
  ...
  DebuggerClient.socketConnect({ host: options.hostname, port: options.port }).then(function (transport) {
    ...
    this._client.connect(function(aType, aTraits) {
      ...
      this._client.addListener("eventNotification", function (aType, aData) {
        if (aData.event in this._eventObservers) {
          for (let observer of this._eventObservers[aData.event]) {
            observer.observe(aData.subject, aData.event, aData.data);
          }
        }
      }.bind(this));
    }.bind(this));
...
RemoteProfiler.prototype = {
  start: function remoteProfiler_start(entries, interval, features, threadFilters, cb) {
    this._client.request({ to: this._profilerActor, type: "startProfiler", entries: entries, interval: interval, features: features }, function(aResponse) {
  ...
  getProfile: function profiler_getProfile(cb) {
    this._client.request({ to: this._profilerActor, type: "getProfile" }, function(aResponse) {
      if (!aResponse.error) {
        if (aResponse.profile.libs) {
          this._sharedLibraryInformation = aResponse.profile.libs;
        }
  ...
  getFeatures: function profiler_getFeatures(cb) {
    // The profiler actor no longer has a getFeatures method.
    cb([]);
  },
  getSharedLibraryInformation: function profiler_getSharedLibraryInformation(cb) {
    // The profiler actor no longer has a getSharedLibraryInformation method.
    // We get the shared library information from the profile, in getProfile().
    cb(this._sharedLibraryInformation);
  },
