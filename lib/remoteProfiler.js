...
function RemoteProfiler(options, cb) {
  ...
  this._client = new DebuggerClient(debuggerSocketConnect(options.hostname, options.port));
...
RemoteProfiler.prototype = {
  start: function remoteProfiler_start(entries, interval, features, cb) {
  ...
  sendObservableEvent: function profiler_sendObservableEvent(eventName, msg) {
    console.log("Can't dispatch observableEvent to remote host. Implement me");
  },
